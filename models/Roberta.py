...
class ManuralHardPromptModel4RoBerta(nn.Module):
...
class PROModel(nn.Module):
    def __init__(self, bert, opt):
        ...
        self.bert2att = nn.Linear(self.vocab_size, self.attdim)
    ...
    def forward(self, inputs):
        ...
        rawoutputs = self.bert(input_ids, attention_mask=attention_mask)
        rawinputs = rawoutputs[0]
        inputs = self.batchnorm(rawinputs)
        inputs = self.bert_drop(inputs)
        inputs = self.bert2att(inputs)

        # get mask token representation
        mask_index = mask_index.unsqueeze(-1).expand(batch, self.attdim).unsqueeze(-1)
        # outputs = attoutputs.reshape(batch, self.attdim, 100)
        outputs = torch.transpose(inputs, 2, 1)
